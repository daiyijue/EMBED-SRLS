...
% H weights
if ~isfield(pars, 'H')
    pars.H = spones(D);%replace the nonzero element to 1 in D
end
H = pars.H;
...
% percentage of missing distances to control to use shortestpaths%the percent of missing distances
...
if (spathyes) && (nnz(D)/(n^2-n) < (1-pcent))  %nnz求非零个数
    fprintf('\n ^^^^^^^ EMBED: Computing the shortest paths ... ^^^^^^^');
    D = graphallshortestpaths(D);  %find the all disthances that are the shortest
...
if (dim == 3)
   ...
   if (m > 0)
    ...
    if (np > m)
      ...
      xlabel(['EMBED: RMSD = ', sprintf('%4.2e', RMSD)]);
     % pause(0.1);
     legend('EMBED','reference nodes','real values',3);
      hold off
end
